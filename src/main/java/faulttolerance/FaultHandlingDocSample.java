...
import static faulttolerance.FaultHandlingDocSample.WorkerApi.*;
import static faulttolerance.FaultHandlingDocSample.CounterServiceApi.*;
import static faulttolerance.FaultHandlingDocSample.CounterApi.*;
import static faulttolerance.FaultHandlingDocSample.StorageApi.*;

//#imports
/**
 * Lifted from https://github.com/akka/akka/tree/master/akka-docs
 * 
 * @author bobk
 * 
 */
public class FaultHandlingDocSample {
    ...
    public static class Worker extends UntypedActor {
        ...
        public void onReceive( Object msg ) {
            ...
            if ( msg.equals( Start ) && progressListener == null ) {
                ...
                pipe(
                        ask( counterService, GetCurrentCount, askTimeout ).mapTo( classTag( CurrentCount.class ) ).map(
    ...
    /**
     * Adds the value received in Increment message to a persistent counter. Replies with
     * CurrentCount when it is asked for CurrentCount. CounterService supervise Storage and Counter.
     */
    public static class CounterService extends UntypedActor {
        ...
        /**
         * The child storage is restarted in case of failure, but after 3 restarts, and still
         * failing it will be stopped. Better to back-off than continuously failing. When it has
         * been stopped we will schedule a Reconnect after a delay. Watch the child so we receive
         * Terminated message when it has been terminated.
         */
        void initStorage() {
        ...
        public void onReceive( Object msg ) {
            ...
            if ( msg instanceof Entry && ( (Entry) msg ).key.equals( key ) && counter == null ) {
                ...
                getContext().system().scheduler()
                        .scheduleOnce( Duration.create( 10, "seconds" ), getSelf(), Reconnect, getContext().dispatcher(), null );
    ...
    /**
     * Saves key/value pairs to persistent storage when receiving Store message. Replies with
     * current value when receiving Get message. Will throw StorageException if the underlying data
     * store is out of order.
     */
